...
func newZendeskCollector(client *zendesk.Client) *ZendeskCollector {
...
func (collector *ZendeskCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if len(os.Getenv("ZENDESK_QUERY")) != 0 {
		tickets, err = collector.client.Ticket().Search(os.Getenv("ZENDESK_QUERY"))
		if err != nil {
			log.Error(err)
		}
	} else {
		tickets, err = collector.client.Ticket().GetAll()
		if err != nil {
			log.Error(err)
		}
	}
...
func main() {
	// Create a new instance of the zendesk client
	...
	zendeskCollector := newZendeskCollector(&client)
